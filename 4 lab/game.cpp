...
bool Game::playerTurn(size_t x, size_t y, bool use_skill, size_t skill_x, size_t skill_y) {
    this->check_game_status();
    bool skills_result = false;
    if (use_skill) {
        this->game_state.info_holder.x = skill_x;
        this->game_state.info_holder.y = skill_y;
        try{
            skills_result = this->game_state.skills_manager.useFront(this->game_state.info_holder);
        }
...
bool Game::isPlayerWin() {
    if (this->game_state.enemy_field->isAllShipsDead()){
        this->game_state.is_game_started = false;
        return true;
    }
    return false;
}
...
PlayingField& Game::getPlayerField() {
    return *this->game_state.player_field;
}

PlayingField& Game::getEnemyField() {
    return *this->game_state.enemy_field;
}

SkillsManager* Game::getSkillsManager() {
    return &this->game_state.skills_manager;
}
